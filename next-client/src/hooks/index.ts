 ...
 */
export const usePagination = <T = any>(initialLimit = 10) => {
    const [items, setItems] = useState<T[]>([]);
    const [cursor, setCursor] = useState<string | null>(null);
    const [hasMore, setHasMore] = useState(true);
    const [loading, setLoading] = useState(false);

    const loadMore = useCallback(async (fetchFn: (c: string | null, l: number) => Promise<{ data: { jobs?: T[], items?: T[], hasMore: boolean } }>) => {
        ...
        try {
            ...
            setCursor((newItems[newItems.length - 1] as any)?._id || null);
