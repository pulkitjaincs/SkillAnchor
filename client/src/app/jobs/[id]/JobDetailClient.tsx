...
export default function JobDetailClient({ id }: { id: string }) {
    ...
    if (isError || !job) {
        return (
            <div className="container py-5 text-center">
                ...
                <Link href="/" className="btn btn-outline-primary mt-3">← Back to Jobs</Link>
            </div>
        );
    }
    ...
    return (
        <div className="container py-4">
            <button onClick={() => router.back()} className="btn text-decoration-none d-inline-flex align-items-center mb-4 p-0"
                        ...
                        <div className="d-flex flex-column flex-sm-row align-items-start gap-3 gap-md-4 mb-4">
                            {job.company?.logo ? (
                                <div className="position-relative flex-shrink-0" style={{ width: '64px', height: '64px' }}>
                                    <Image src={job.company.logo} alt={job.company.name}
                                        fill sizes="64px" className="rounded-4 shadow-sm"
